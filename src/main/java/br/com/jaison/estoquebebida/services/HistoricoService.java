...
public class HistoricoService {
	...
	private void validateHistoricoFields(Historico historico) throws RegraDeNegocioValidationException {
		if (historico == null) {
			throw new RegraDeNegocioValidationException(
					"Por favor insira dos dados para processar a operacao de estoque");
		}
	 ...
	 * 
	 *             Registro deve conter horário, tipo, volume, seção e responsável
	 *             pela entrada
	 * 
	 * 
	 */
	public Historico executaOperacaoEstoque(Historico operacaoEstoqueHistorico, Integer numeroSecao)
		...
		serviceEstoque.isSecaoExists(numeroSecao);

		validateHistoricoFields(operacaoEstoqueHistorico);
		serviceEstoque.ValidateEstoque(numeroSecao, operacaoEstoqueHistorico.getItemEstoque());
		validateOperacaoEstoque(operacaoEstoqueHistorico);
		return repository.save(operacaoEstoqueHistorico);
	 ...
	 * foram consumidas do estoque, só poderá receber não alcoólicas no dia
	 * seguinte.
	 * 
	 * @throws RegraDeNegocioValidationException
	 */
	private void validaPeriodoEstocagemTipoBebidaAlcoolica(Historico operacaoEstoqueHistorico)
			throws RegraDeNegocioValidationException {
		if (operacaoEstoqueHistorico.getItemEstoque().getBebida().getTipo()
				.equals(TipoBebidaTypes.NAO_ALCOOLICA.value())) {
			...
			/*
			 * Verifica se houve movimentacao na determinada secao com bebidas do tipo
			 * alcoolica
			 */
			List<Historico> historicoOperacoesComBebidasAlcoolicasNessaSecao = repository
					.findByItemEstoqueNumeroSecaoEqualsAndItemEstoqueBebidaTipoEqualsAndDataOperacaoAfter(
							operacaoEstoqueHistorico.getItemEstoque().getNumeroSecao(),
							TipoBebidaTypes.ALCOOLICA.value(), dataOper);
	...
	private void validateOperacaoEstoque(Historico operacaoEstoqueHistorico)
			throws RegraDeNegocioValidationException, NotFoundObjectException {
		...
		if (operacaoEstoqueHistorico.getOperacaoEstoque().equals(OperacaoEstoqueType.ENTRADA.value())) {
			...
			serviceEstoque.saveEstoque(operacaoEstoqueHistorico.getItemEstoque());
		} else if (operacaoEstoqueHistorico.getOperacaoEstoque().equals(OperacaoEstoqueType.SAIDA.value())) {
			if (operacaoEstoqueHistorico.getItemEstoque().getId() == null
					|| operacaoEstoqueHistorico.getItemEstoque().getId().isEmpty()) {
				throw new RegraDeNegocioValidationException(
						"Informe o ID para realizar a operacao de saida do item de estoque");
			}

			serviceEstoque.deleteById(operacaoEstoqueHistorico.getItemEstoque().getId());
	...
	public List<Historico> listarHistoricosPorSecaoDataOperacao(boolean asc) {
		if (asc) {
			return repository.findAll(Sort.by(new Order(Direction.ASC, "itemEstoque.numeroSecao"),
					new Order(Direction.ASC, "dataOperacao")));
		} else {
			return repository.findAll(Sort.by(new Order(Direction.DESC, "itemEstoque.numeroSecao"),
					new Order(Direction.DESC, "dataOperacao")));
		}
	}
